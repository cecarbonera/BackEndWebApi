...
namespace BackEndWebApi.Controllers
{
    ...
    public class DepartamentoController : ControllerBase
    {
        ...
        public async Task<ActionResult> Listar()
        {
            try
            {
                return Ok(_conexao.Query<Departamento>("SELECT CODIGO, DESCRICAO FROM DBO.DEPARTAMENTO ORDER BY DESCRICAO"));
        ...
        public async Task<ActionResult> ListarDeptoEmpr()
        {
            try
            {
                return Ok(_conexao.Query<Departamento>("SELECT CODIGO, CONCAT(CAST(CODIGO AS VARCHAR(2)), ' - ', DESCRICAO) AS DESCRICAO FROM DBO.DEPARTAMENTO"));
        ...
        public async Task<ActionResult> Inserir([FromForm] Departamento departamento)
        {
            try
            {
        ...
        [Route("Atualizar")]
        [Authorize(Roles = "D,G,F")]
        public async Task<IActionResult> Atualizar([FromForm] Departamento departamento)
        {
            try
            {
                if (departamento == null)
